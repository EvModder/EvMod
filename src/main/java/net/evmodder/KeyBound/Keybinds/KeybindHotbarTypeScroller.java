...
public final class KeybindHotbarTypeScroller{
	...
	private void scrollHotbarSlot(boolean upOrDown){
		...
		assert PlayerInventory.isValidHotbarIndex(inventory.getSelectedSlot());
		...
		if(client.player.isInCreativeMode()){
			...
			do{
				int j = 0;
				for(; j<PlayerInventory.MAIN_SIZE; ++j){
				...
				if(j != inventory.getMainStacks().size()){
					...
					if(PlayerInventory.isValidHotbarIndex(j)) inventory.setSelectedSlot(j);
						...
						client.interactionManager.clickSlot(0, j, inventory.getSelectedSlot(), SlotActionType.SWAP, client.player);
					}
