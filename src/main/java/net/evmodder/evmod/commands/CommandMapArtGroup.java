...
public class CommandMapArtGroup{
	...
	public final HashSet<UUID> getGroupIdsOrSendError(final FabricClientCommandSource source, final String... groups){
		...
		for(int i=0; i<groups.length; ++i) data[i] = FileIO_New.loadFileBytes(FILE_PATH+groups[i]);
...
//			source.sendFeedback(Text.literal("MapArtGroups "+groupName1+" and "+groupName2+" are identical").withColor(DONE_COLOR));
		...
		if(!in1Not2.isEmpty()){
			...
			FileIO_New.saveFileBytes(FILE_PATH+in1Not2Name, bb1.array());
			if(in2Not1.isEmpty()){
...
//						+"' and set as active (ids: "+colorIds1.size()+")").withColor(CREATE_COLOR));
		...
		if(!in2Not1.isEmpty()){
			...
			FileIO_New.saveFileBytes(FILE_PATH+in2Not1Name, bb2.array());
//			if(in1Not2.isEmpty()){
...
//			source.sendError(Text.literal("Command requires a single MapArtGroup name (no commas)").withColor(ERROR_COLOR));
		...
		if(cmd == Command.CREATE && new File(FileIO_New.DIR+FILE_PATH+groups[0]).exists() && (groups2 == null || !CONFIRM.equalsIgnoreCase(groups2[0]))){
...
//				source.sendError(Text.literal("No new maps found for group '"+newActiveGroup+"'").withColor(DONE_COLOR));
				...
				if(FILE_PATH.endsWith("/") && !new File(FileIO_New.DIR+FILE_PATH).exists()) new File(FileIO_New.DIR+FILE_PATH).mkdir();
				FileIO_New.saveFileBytes(FILE_PATH+groups[0], bb.array());
...
//			source.sendFeedback(Text.literal("Set active group: '"+newActiveGroup+"' (ids: "+mapsInGroup.size()+").").withColor(DONE_COLOR));
	...
	private CompletableFuture<Suggestions> getGroupNameSuggestions(CommandContext<?> ctx, SuggestionsBuilder builder) {
		...
		if(ctx.getArgument("command", String.class).equalsIgnoreCase(Command.CREATE.translation) || !new File(FileIO_New.DIR+FILE_PATH).exists()){
		...
		try{
			Files.list(Paths.get(FileIO_New.DIR+FILE_PATH))
						...
						.then(
							...
							.suggests((ctx, builder) -> {
								...
								if(cmdStr.equalsIgnoreCase(Command.COMPARE.translation)) return getGroupNameSuggestions(ctx, builder);
										...
										&& new File(FileIO_New.DIR+FILE_PATH+ctx.getArgument("group", String.class)).exists()) builder.suggest(CONFIRM);
								return builder.buildFuture();
							})
							.executes(ctx->{
								final String cmdStr = ctx.getArgument("command", String.class);
